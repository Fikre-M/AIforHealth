...
export const createConversation = asyncHandler(async (req: Request, res: Response) => {
  ...
  try {
    ...
    console.error('[createConversation] AI error:', e?.message);
    res.status(500).json({
      success: false,
      message: `AI service error: ${e?.message ?? 'Unknown error'}`,
    });
  }
});
...
export const sendMessage = asyncHandler(async (req: Request, res: Response) => {
  ...
  try {
    ...
    const e = error as { message?: string; status?: number };
    console.error('[sendMessage] AI error:', e?.message);
    // Distinguish between "conversation not found" and AI failures
    const isNotFound = e?.message?.toLowerCase().includes('not found');
    res.status(isNotFound ? 404 : 500).json({
      success: false,
      message: isNotFound
        ? 'Conversation not found'
        : `AI service error: ${e?.message ?? 'Unknown error'}`,
    });
  }
});
...
export const getConversations = asyncHandler(async (req: Request, res: Response) => {
  ...
  const result = await AIAssistantService.listConversations(new Types.ObjectId(req.user.userId), {
...
export const deleteConversation = asyncHandler(async (req: Request, res: Response) => {
  ...
  const deleted = await AIAssistantService.deleteConversation(
    conversationId,
    new Types.ObjectId(req.user.userId)
  );
...
export const checkSymptoms = asyncHandler(async (req: Request, res: Response) => {
  ...
  const response = {
    ...
    disclaimer:
      'This is not a medical diagnosis. Please consult with a healthcare professional for an accurate assessment of your symptoms.',
    isEmergency: false,
  ...
  res.status(200).json({
    ...
    meta: {
      disclaimer:
        'This is a placeholder response. In a real implementation, this would be generated by an AI model analyzing your symptoms.',
    },
  });
});
